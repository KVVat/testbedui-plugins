...
* **Final Determination (2026-04-23): Not achievable from a standard Android application.** Every stable TLS stack reachable from `HttpURLConnection` / `OkHttp3` — both the platform Conscrypt baked into Android 17 and the latest **bundled** `org.conscrypt:conscrypt-android:2.5.3` (released 2024-08-18, still the latest on Maven Central) — omits `secp384r1 (0x0018)` from the on-wire `supported_groups` list. The experimental bundled-Conscrypt integration used to confirm this was reverted from `apps/openurl/` to keep the TOE representative of a conventional Android app.
* **Experiment Results (2026-04-23):**
  * **Experiment A — platform Conscrypt + `SSLParameters.setNamedGroups` via reflection:** The `Supported Groups` list changed and included `X25519MLKEM768 (0x11EC)` plus a GREASE code, but `secp384r1` was still not sent. Other required extensions like `psk_key_exchange_modes (0x002D)` disappeared from the `ClientHello` as a side-effect (SSLParameters customization resets Conscrypt's implicit defaults).
  * **Experiment B — `SSLContext.getInstance("TLS", "Conscrypt")` + `setNamedGroups` + `setSignatureSchemes` via reflection:** Almost all extensions disappeared from the `ClientHello` except `key_share (0x0033)` and a GREASE code. Unviable.
  * **Experiment C — Bundle latest stable Conscrypt (`org.conscrypt:conscrypt-android:2.5.3`) and register as Provider 1:** ClientHello became richer (GREASE, `X25519MLKEM768`, ECH `0xFE0D`, trust_anchors `0x44CD`, certificate_compression `0x001B`) and `signature_algorithms` gained `ecdsa_secp384r1_sha384 (0x0503)` and `rsa_pkcs1_sha384 (0x0501)` (CNSA-compliant), but **`supported_groups` remained `[GREASE, X25519MLKEM768, x25519, secp256r1]` — still no `secp384r1`**.
  * **Experiment D — Bundled Conscrypt 2.5.3 + `SSLParameters.setNamedGroups(["secp384r1","secp256r1","x25519"])` via a wrapping `SSLSocketFactory`:** The reflection call succeeded (`Named groups forced` logged) but on-wire `supported_groups` was `[x25519, secp256r1]` only (secp384r1 silently dropped) **and** the handshake fell back to TLS 1.2 (supported_versions/GREASE/PQ all lost). Actively harmful — reverted.
* **Root Cause:** A source audit of Conscrypt 2.5.3 confirms `SSLParametersImpl.java` has **zero** references to `namedGroups` — the API itself is not wired up in the 2.5.3 release. The mapping `"secp384r1" → NID_secp384r1` only exists in Conscrypt's `master` branch (`NativeSsl.toBoringSslGroup`), which is **unreleased and unstable**. No artifact later than `2.5.3` exists on Maven Central as of 2026-04-23 (`conscrypt-openjdk`/`-uber` are stuck at `2.5.2` since 2021). Chrome works around this by compiling its own BoringSSL with an independent client hello policy, i.e. it **does not** use Conscrypt at all.
* **Conclusion:** From the perspective of a conventional Android app using the public HTTPS client APIs (`HttpURLConnection`, `OkHttp3`), **presenting `secp384r1` in `supported_groups` is not possible with any current stable release**. The only theoretical paths forward are (a) build Conscrypt from `master` and ship the unstable native library with the app — which steps outside the "standard app development" scope this TOE represents — or (b) wait for Conscrypt ≥ 2.5.4. Accordingly the CNSA `secp384r1` wire requirement is treated as an **out-of-scope limitation** at the HTTPS-client layer.
* **Test Assertion:** `FcsTlscExtTest.analyzePcap` uses `secp384r1 OR secp256r1` and passes on current Conscrypt (via `secp256r1`). This matches the ST revision needed.
