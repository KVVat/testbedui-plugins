 ...
 * - **OCSP Mock Servers:** Requires running Python HTTPS mock servers (ports 4443-4448) and an OpenSSL OCSP responder (port 8888) on the host PC. Run the startup script `scratch/start_ocsp_mocks.sh` in the TestBed Core repository.
 * - **CA Trust Store:** Requires importing the root CA certificates (`root-ca.crt` files) generated by `gen-ocsp-certs.sh` into the target Android device's trust store (Settings -> CA Certificate).
 * - **Port Forwarding:** Prior to execution, all mock server ports must be forwarded to the host:
 *   `adb reverse tcp:4443 tcp:4443` through `4448` and `8888`.
 */
@SFR("FIA_X509_EXT", """
...
class FiaX509RevocationTest {
    ...
    private val REVERSE_PORTS = listOf(4443, 4444, 4445, 4446, 4447, 4448, 8888, 8889, 8890, 8891)
    ...
    fun setup() {
        ...
        for (port in REVERSE_PORTS) {
            try {
                ...
                log("adb reverse set for port $port")
            } catch (e: Exception) {
                log("adb reverse failed for port $port: ${e.message}")
            }
        }
    }
    ...
    fun teardown() {
        ...
        for (port in REVERSE_PORTS) {
            try {
                ProcessBuilder("adb", "-s", serial, "reverse", "--remove", "tcp:$port").start().waitFor()
                log("adb reverse removed for port $port")
            } catch (e: Exception) {
                log("adb reverse remove failed for port $port: ${e.message}")
            }
        }
    }
...
//            Assert.assertTrue("Failed to install openurl-niapsec app: ${ret}", ret.startsWith("Success"))
            ...
            val res = AdamUtils.waitLogcatLine(400, "worker@return", adb)
            if (res != null) {
                val text = res.text
