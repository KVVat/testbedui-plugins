...
DIR="/Users/wkouki/AndroidStudioProjects/testbedui-plugins/test-sample/src/main/resources/revocation/sha512"
cd $DIR

# Generate Root CA Key (EC P-384)
openssl ecparam -name secp384r1 -genkey -out root-ca.key
# Generate Root CA Cert
openssl req -new -x509 -days 3650 -key root-ca.key -out root-ca.crt -subj "/CN=Test SHA512 Root CA" -sha512
...
# Sign Server Cert
openssl x509 -req -days 365 -in server-valid.csr -CA root-ca.crt -CAkey root-ca.key -CAcreateserial -out server-valid.crt -sha512 -extfile server.ext

# Generate Responder Key (EC P-384) and CSR
openssl ecparam -name secp384r1 -genkey -out responder.key
openssl req -new -key responder.key -out responder.csr -subj "/CN=Test SHA512 OCSP Responder"

# Responder Extensions
echo "extendedKeyUsage=1.3.6.1.5.5.7.3.9" > responder.ext

# Sign Responder Cert
openssl x509 -req -days 365 -in responder.csr -CA root-ca.crt -CAkey root-ca.key -CAcreateserial -out responder.crt -sha512 -extfile responder.ext
